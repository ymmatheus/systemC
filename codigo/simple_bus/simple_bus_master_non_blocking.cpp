  ...
  while (true)
    {
		...
		cout << " =================== > Send X: " << sc_simulation_time() << endl;
		
		bus_port->write(m_unique_priority, &mydata, X_ADDRESS, m_lock);
		...
		cout << " =================== > Send Y: " << sc_simulation_time() << endl;
		
		mydata = 5;		
		bus_port->write(m_unique_priority, &mydata, Y_ADDRESS, m_lock);
		while ((bus_port->get_status(m_unique_priority) != SIMPLE_BUS_OK) &&
			   (bus_port->get_status(m_unique_priority) != SIMPLE_BUS_ERROR))
			wait();
		if (bus_port->get_status(m_unique_priority) == SIMPLE_BUS_ERROR)
		...
		cout << " =================== > Send start: " << sc_simulation_time() << endl;
		
		mydata = START_MULT;
		bus_port->write(m_unique_priority, &mydata, START_ADDRESS, m_lock);
		while ((bus_port->get_status(m_unique_priority) != SIMPLE_BUS_OK) &&
			   (bus_port->get_status(m_unique_priority) != SIMPLE_BUS_ERROR))
			wait();
		if (bus_port->get_status(m_unique_priority) == SIMPLE_BUS_ERROR)
		...
		bus_port->read(m_unique_priority, &mydata, Z_ADDRESS, m_lock);
		while ((bus_port->get_status(m_unique_priority) != SIMPLE_BUS_OK) &&
			   (bus_port->get_status(m_unique_priority) != SIMPLE_BUS_ERROR))
			wait();
		...
		cout << "End multiplication: " << sc_simulation_time() << endl;
